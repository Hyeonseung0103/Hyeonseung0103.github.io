...
AUC가 0.5라는 것은 TPR과 FPR의 증가하는 양이 똑같아
서 원점에서 정확히 대각선으로 그려지게 될 것이다. 이는 곧 특이도가 1일 때(다 0으로 찍은 것) 민감도는 0이 되고, 민감도가 1일 때(다 1로 찍은 것) 특이도가 0이 되는 것을 의미한다.
AUC가 0.5 = 특이도와 민감도의 합이 항상 1인 관계 = 두 값이 정확하게 trade off 되는 관계.
