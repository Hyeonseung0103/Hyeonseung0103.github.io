...
위의 그림처럼 퍼셉트론은 입력층(x)을 제외하면 가중치-편향 연산(w)과 활성화 함수(y), 이 두 부분으로 나눌 수 있다. y는 결과값을 의미하지만, 결과값을 도출하기 위해 사용하는 것이
...
!퍼셉트론은 다수의 입력을 받아 하나의 출력값을 내보내는 구조이다. 단층 퍼셉트론은 은닉층이 없는 퍼셉트론이라고 했는데 여기서 중요한 점은 가중치-편향 연산이 꼭 **은닉층에서만
수행되는 것이 아니다** 입력층에서 출력층으로 바로 전달되는 단층 퍼셉트론도 가중치-편향 연산이 수행되어 활성화함수를 거쳐서 출력값을 도출할 수 있다. 단층이든, 다층이든 퍼셉트론은 가중치-편향 연산, 활성화 함수 이 두 가지가 단계로 이루어져있다는 사실을 기억하자. 

### 활성화 함수(Activation Function)
활성화 함수는 입력층이나 은닉층으로부터 가중합을 전달 받아서 전달받은 정보를 회귀 or 분류 문제에 알맞은 적절한 타겟값으로 변환시키는 함수이다.
...
위의 그래프를 보면 임계값보다 작은 부분은 임계값과의 차이가 클수록 출력값이 0에 가까워지고, 임계값보다 큰 부분은 차이가 클수록 1에 가까워진다.
...
소프트맥스 함수는 다중 분류 문제에 적용할 수 있도록 시그모이드 함수를 일반화시킨 활성화 함수이다. 가중합을 소프트맥스 함수에 통과시키면 해당 입력이 각 클래스일 확률이 얼마나 되는지
...
은닉층은 입력층 혹은 이전 은닉층으로부터 입력된 신호가, 가중치 편향과 함께 연산되는 층이다. 일반적으로는 입력층과 출력층 사이에 있는 층을 은닉층이라고 부르고, 은닉층에서 일어나는 계산의 결과를 사용자가 볼 수 없어서 은닉층이라고 한다. 은닉층은 데이터셋의 특성 수와 상관없이 노드 수를 구성할 수 있다.
