...
층이 깊어질수록 앞에 있는 층에는 손실 정보가 잘 전달되지 않는 기울기 소실(Vanishing Gradient)이 발생한다. 반대로, 미분값이 조금만 커도 층이 깊어질수록 그 미분값이 제곱의 제곱의 제곱이 되어서 앞층에는 손실 정보가 과하게 전달될 수 있는 기울기 폭발(Exploding Gradient)이 발생할 수도 있다.
...
GRU에서는 output gate가 없어지고 전체 상태 벡터인 $ h_{t} $ 각 time-step 에서 출력되며, 이전 상태의 $ h_{t-1} $ 중 어느 부분이 출력할 지 새롭게 제어하는 Gate인 $ r_{t} $ 가
추가 되었다.

GRU가 파라미터 개수가 LSTM에 비해 현저하게 감소한 것이지만, 오늘날은 컴퓨터가 많이 발전해서 굳이 기울기 소실이 좀 더 불확실한 GRU를 쓸 필요가 없다. 과거에 컴퓨터가 과도한 연산량을
처리하기가 버거울 때는 GRU를 쓰는 것이 더 적합했지만, 현재는 복잡한 연산을 더 잘 수행할 수 있어서 가중치를 빼가면서까지 GRU를 사용할 필요는 없다고 한다.

LSTM과 GRU는 기울기 소실 문제를 개선했지만, 결국 Vanilla RNN처럼 Sequence를 활용하여 문제를 해결하기 때문에 병렬화를 사용할 순 없다.
...
매우 길어지면 모든 단어 정보를 고정된 hidden-state 벡터에 담는 것이 어렵다. 
...
Value: Key에 대한 값, 의미를 보여준다. (비슷한 단어들의 의미(값)은 이거야. 인코더가 디코더에게 키에 알맞은 값을 알려줌)
...
1) 인코더에서 RNN으로 각 한국어 단어의 정보를 hidden state에 저장한다. (time-step마다 정보 저장)
