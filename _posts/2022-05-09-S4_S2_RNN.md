...
GRU에서는 output gate가 없어지고 전체 상태 벡터인 $ h_{t} $ 각 time-step 에서 출력되며, 이전 상태의 $ h_{t-1} $ 중 어느 부분을 출력할 지 새롭게 제어하는 Gate인 $ r_{t} $ 가
추가 되었다.

GRU는 파라미터 개수가 LSTM에 비해 현저하게 감소한 것이지만, 오늘날은 컴퓨터가 많이 발전해서 굳이 기울기 소실이 좀 더 불확실한 GRU를 쓸 필요가 없다. 과거에 컴퓨터가 과도한 연산량을
