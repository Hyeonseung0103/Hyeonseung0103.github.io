...
기울기는 가중치($x$)가 변할 때 손실($y$ = $f(x$<sub>i</sub>$)$) 가 어떻게 변하는지를 나타내고, 손실함수는 다음과 같이 표현할 수 있다.

$f(x) =$ $\frac{1}{2}$$(y-(wx+b))$<sup>2</sup>

$\frac{1}{2}$은 계산편의를 위한 것이고, $(wx+b)$는 가중치 연산을 통해 나온 예측값($y'$)이다. 위의 식에서 업데이트 하고자 하는 것은 가중치이므로 y,w,x,b로 이루어진 다변함수인
손실함수를 
...
![image](https://user-images.githubusercontent.com/97672187/165676047-edc832df-e7b3-48f2-ba39-12cc7e349f3a.png){: .align-center}
