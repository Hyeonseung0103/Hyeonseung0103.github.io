...
- $1_{ij}^{obj}$는 grid cell내에 class가 존재한다는 것을 의미하고 존재하면 1, 아니면 0으로 표현한다. 0은 $1_{ij}^{noobj}$와 같다.
- $1_{ij}^{obj}$는 grid cell내에 class가 존재한다는 것을 의미하고 존재하면 1, 아니면 0으로 표현한다. 0은 $1_{a}^{a}$와 같다.
