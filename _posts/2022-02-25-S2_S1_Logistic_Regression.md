...
![image](https://user-images.githubusercontent.com/97672187/155836582-a3b05d4f-2c62-456d-8293-1c3be3774854.png){: .align-center}
...
하지만, 이는 회귀분석처럼 타겟값의 수치가 커지고, 작아지는 것이 아니라 특정 범주에 속할 확률을 높이고 낮추는 것으로 해석해야한다.

계수를 Odds 확률로 해석하려면 exp(계수) = k 를 계산하고 특성 1단위에 증가당 Odds확률이 k배 증가한다고 해석할 수 있다. 로짓변환했으니까 로그를 원래값으로 다시 돌려주기 위해 exp().

타이타닉 데이터를 예제로 들면 타겟이 생존여부인데 로짓변환을 통해 변환시킨 회귀계수가 양수이고 exp로 바꾼 Odds 비가 1 이상이면 생존의 가능성을 높이고, 1 이하면 생존의 가능성을 낮춘다고 해석할 수 있다. 회귀계수가 음수면 모두 반대로 해석.

나이에 대한 회귀계수가 3이라면 여기에 exp를 씌워서 k를 계산하고 만약 k가 0.5라면 나이가 1 증가할 때마다 생존할 확률이 0.5%(0.5 - 1 = -0.5) 감소한다고 할 수 있다.
k가 1.5라면 생존할 확률이 0.5%(1.5-1 = 0.5) 증가한다고 할 수 있다.

회귀계수가 -3이고, k가 0.5라면 나이가 1 증가할수록 생존율이 (0.5 - 1 = -0.5, 하지만, 회귀계수가 음수니까 반대로 해서 0.5로 나옴) 0.5% 증가한다.
반대로 k가 1.5면 나이가 1 증가할수록 생존율이 0.5%(1-1.5 = 0.5, 근데 회귀계수가 음수니까 반대로 -0.5) 감소한다.

회귀계수 해석

1) 회귀계수에 exp() 씌워서 원래대로 변환 = k

2) (k - 1) * 100 % 로 결과를 해석하는데 회귀계수가 음수였다면 결과를 반대로 해석한다.

3) 하지만 계수가 1 감소할 때는 다른 공식을 써야한다.( 1번에서 구한 k를 분모로 보내야함. 1/k가 되도록 )

예를 들어, 남자가 1 이라고 하면, 여자에 비한 남자의 생존률은 그대로 (k-1) * 100 이지만, 남자에 비한 여자의 생존률(즉, 기준이 아닌 0의 값을 알고 싶음. 숫자로는 1 감소)을
구하려면 (1/k - 1) * 100으로 해석해야함.
