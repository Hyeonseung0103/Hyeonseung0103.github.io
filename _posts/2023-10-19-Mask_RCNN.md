...
- Faster R-CNN은 1 stage는 RPN, 2 stage는 RoIPool을 통해 특징을 추출하고 분류 및 box regression을 수행하는 Fast R-CNN으로 이루어져있다. Mask R-CNN도 2 stage 모델로 1 stage는 RPN, 2 stage는 class/box offsets/binary mak tasks를 각 RoI에 대해 병렬적으로 예측한다. 이것은 classification에 의존한 mask predictions을 사용하는 최근 모델들과는 다르다는 것을 보여준다.
...
- Figure 3.을 보면 점선이 피처맵, 실선이 RoI(2x2 bin을 가진)라고 할 때, 더 정확한 RoI 피처맵을 만들기위해 각각의 bin을 4개의 sampling points를 사용해서 또 4등분 하여 subsample을 만들고 이 subsample에 대해 bilinear inpolation을 적용하고 최종적으로 max pooling이나 average pooling을 적용해 특징을 추출하는 것이다.
...
- Mask R-CNN의 일반화 능력을 입증하기위해 다양한 architecture를 조합하여 사용했다.
  - 전체 이미지에 대해 특징을 추출한 cnovolutional backbone architecture
...
- 이미지를 resizing, mini-batch 2 images per GPU, N RoIs per image, 1:3 positive:negative ratio가 적용됐다. N은 C4 backbone에서는 64, FPN에서는 512로 적용했다.
- 8 GPUs(mini batch 2 * 8 = 16), 160k iterations, lr 0.02(120k iteration에 10배 감소), weight decay 0.0001, momentum 0.09.
- RPN anchors box는 5 scales과 3 aspect ratios를 가졌고 RPN은 명시하지않는 한 Mask R-CNN과 별개로 분리되어 학습되어 features를 공유하지 않지만, 본 연구에서는 두 네트워크가 backbone이 같기때문에 공유할 수 있다.
...
- Mask branch는 RoI당 K(class 갯수)개의 masks를 예측할 수 있지만 모든 class에서의 mask가 아니라 classification branch에서 예측한 k라는 class에 대해서만 mask 예측을 수행한다.
...
- Table 1.을 통해 Mask R-CNN은 모든 AP scale에서 이전 SOTA 모델들을 뛰어넘었다. 종과 호루라기 객체말고는 RestNet-101-FPN backbone모델이 multi-scale train/test, horizontal flip test, online hard example mining등 다양한 기법이 적용된 FCIS+++을 뛰어넘었다.
...
<div align="center">
  <p>
  <img width="800" alt="image" src="https://github.com/Hyeonseung0103/Hyeonseung0103.github.io/assets/97672187/2ca46f6a-bb37-487f-bfae-48d5dd174ab2">
  </p>
</div>
...
**Architecture**
- Mask R-CNN은 다양한 backbones을 사용했고 깊은 네트워크(50 vs 101)와 FPN, ResNeXt의 advanced 모델들이 성능 향상에 도움이 됐다(Table 2a). 하지만, 모든 프레임워크가 자동으로 깊고 advanved한 네트워크에서 이점을 누릴 수 있는 것은 아니다.

<br>

**Multivomial vs Independent Masks**
- Table 2b에서는 FCN처럼 pixel 단위로 softmax와 multinomial loss를 사용하느냐 Mask R-CNN처럼 sigmoid와 binary loss를 사용하느냐로 비교를 진행했다. 쉽게 말해서, classificaiton과 mask과 couple이나 decouple이냐의 비교다.
...
**RoIAlign**
- RoIAlign의 효과를 검증할 때는 ResNet-50-C4 backbone(stride 16)을 사용했고 RoIAlign을 RoIPool보다 3 points 높은 AP_75를 기록했다(Tabel 2c).
...
- 추가적으로 ResNet-50-C5 backbone(stride 32)을 설계해 RoIAlign을 비교해보았는데 결과적으로 stride가 더 큰 C5 backbone모델의 AP가 C4보다 높았다(Tabel 2c, 2d). 아무래도 stride가 커져서 misalignment가 더 심해졌을것이고 이에 따라 RoIAlign의 효과가 커졌을 것으로 판단된다. 이를 통해, RoIAlign은 stride가 클수록 detection과 segmentation을 더 잘 수행할 수 있게한다는 것을 알 수 있다.
...
**Mask Branch**
- Segmentation은 FCN을 이용한 pixel-to-pixel task를 수행하는데 FCN의 효과를 확인하기위해 FCN과 MLP(multi layer perceptions)을 비교했다(ResNet-59-FPN backbone 사용, Table 2e).
...
- Mask R-CNN에서 mask와 box AP의 성능차이가 크지않는 것을 보아 Mask R-CNN은 object detection과 더 어려운 instance segmentation task의 갭을 크기 줄였다고 할 수 있다(Table 1, 3 비교).

<br> 
<div align="center">
  <p>
  <img width="500" alt="image" src="https://github.com/Hyeonseung0103/Hyeonseung0103.github.io/assets/97672187/6a6ef5b0-52fa-4127-b0c0-7adeb18cd094">
  </p>
</div>
...
### 4. Timing
**Inference & Training**
- RPN과 Mask R-CNN stages과 features share하도록 ResNet-101-FPN 모델을 사용했을 때 이미지당 195ms의 처리시간이 소요됐다. ResNet-101-C4는 무겁기때문에 더 오랜 시간이 소요돼서 추천하지 않는다.
...
- 새로운 tasks를 수행하기위해 한 instance의 K개의 key points에 대해 training target을 mxm mask 중 하나의 pixel만 foreground(key points)로 라벨링된 ong-hot mxm binary mask을 예측하는 것으로 만들었다. Instance segmentation처럼 K keypoints로 서로 독립적으로 예측된다.
...
- ResNet-50-FPN으로 Key point에 대한 AP를 평가지표로 사용했을 때 2016 COCO keypoint detection winner보다 높은 성능을 기록했다. Mask R-CNN이 더 간단하고 빠르다.
