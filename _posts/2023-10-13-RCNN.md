...
### 1. Dataset overview
- 학습 약 39만개, 검증 약 2만개, 테스트 약 4만개로 이루어져있음.
- 검증과 테스트 데이터에는 라벨링이 잘 되어있지만 학습 데이터에는 모든 라벨들에 대해 라벨링이 잘 되어있진 않다.
- 주로 검증 데이터에 의존하고, 학습 데이터는 보조로 사용한다. 검증 데이터를 동일한 크기를 갖게 val1, val2로 나누어서 학습과 검증에서 사용한다.
...
- Hard negative mining(모델이 FP로 잘못 예측한 데이터를 select하여 학습에 다시 사용하는 방법)은 val1으로부터 5000개의 샘플을 뽑아 진행했다.
...
- CPMC regions의 features을 계산하기 위해 3가지 방법을 사용했다.
